                                 ...
                                 const char *value)
{
    ...
    printf("###%s: check point, ext_type %d\n", __FUNCTION__, ext_type);
                            ...
                            STACK_OF(X509_EXTENSION) **sk)
{
    ...
    printf("###%s: nval number %d\n", __FUNCTION__, sk_CONF_VALUE_num(nval));
    for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
        val = sk_CONF_VALUE_value(nval, i);

        printf("###%s: val name: %s, val value %s\n", __FUNCTION__, val->name, val->value);
